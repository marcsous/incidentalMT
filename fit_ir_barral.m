...
%  lambda (optional) is an "ideality" constraint (scalar)
%
...
[R1,~,~,~,~,H] = fminunc(@(R)myfunc(R,TI,TR,data,lambda),R1,opts);

% 95% confidence interval
[resnorm f x phi] = myfunc(R1,TI,TR,data,lambda);
if isreal(data)
...
function [resnorm f x phi] = myfunc(R,ti,tr,b,lambda)
